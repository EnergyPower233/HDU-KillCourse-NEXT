...
pub fn list_runs(directory: &Path) -> Result<Vec<RunInfo>, String> {
    ...
    for entry in fs::read_dir(directory).map_err(|e| e.to_string())? {
        ...
        let info: RunInfo =
            serde_json::from_str(&header).map_err(|_| "运行日志头损坏".to_string())?;
...
pub fn read_run(directory: &Path, id: &str, before: Option<usize>) -> Result<RunPage, String> {
    use std::io::BufRead;
    if !valid_id(id) {
        return Err("运行编号不合法".into());
    }
    let file = File::open(directory.join(format!("{id}.jsonl")))
        .map_err(|_| "找不到运行日志".to_string())?;
    ...
    while before.is_none_or(|limit| count < limit) {
        ...
        let event: Progress =
            serde_json::from_str(&line).map_err(|_| format!("第 {} 条日志损坏", count + 1))?;
    ...
    Ok(RunPage {
        run,
        events: events.into(),
        next_before: (first > 0).then_some(first),
        warning,
    })
}
...
mod tests {
    ...
    fn each_run_keeps_its_own_events_after_reopening() {
        let directory =
            std::env::temp_dir().join(format!("hdu-log-test-{:016x}", rand::random::<u64>()));
        let mut first = RunLog::create(&directory, "清单一").unwrap();
        let first_id = first.info.id.clone();
        for n in 0..1005 {
            first
                .append(&Progress::new("course", "waiting", &n.to_string()))
                .unwrap();
        }
        drop(first);
    ...
    fn history_pages_survive_restart_and_ignore_partial_tail() {
        let directory =
            std::env::temp_dir().join(format!("hdu-log-pages-{:016x}", rand::random::<u64>()));
        let mut log = RunLog::create(&directory, "历史清单").unwrap();
        let id = log.info.id.clone();
        for n in 0..503 {
            log.append(&Progress::new("id", "success", &n.to_string()))
                .unwrap();
        }
        drop(log);
        ...
        let mut file = OpenOptions::new()
            .append(true)
            .open(directory.join(format!("{id}.jsonl")))
            .unwrap();
    ...
    fn unwritable_log_location_is_reported() {
        let path =
            std::env::temp_dir().join(format!("hdu-log-file-{:016x}", rand::random::<u64>()));
