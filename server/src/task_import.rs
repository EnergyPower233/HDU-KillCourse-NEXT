...
impl ImportedCourse {
    fn into_course(self) -> Result<Course, String> {
        if [&self.jxbmc, &self.kch_id, &self.jxb_id].iter().any(|value| value.trim().is_empty()) {
            return Err("课程缺少教学班编号、课程内部编号或教学班内部编号".into());
        }
        Ok(Course {
            jxbmc: self.jxbmc, kch_id: self.kch_id, jxb_id: self.jxb_id,
            kcmc: self.kcmc, kklxmc: self.kklxmc, sksj: self.sksj,
            jxbzc: self.jxbzc, jzgxx: self.jzgxx, jxdd: self.jxdd,
        })
    }
}
    ...
    for list in file.lists {
        ...
        for task in list.tasks {
            tasks.push(CourseTask {
                ...
                drops: task.drops.into_iter().map(ImportedCourse::into_course).collect::<Result<_, _>>()?,
            });
        }
        lists.push(TaskList { name: list.name, mode: list.mode, tasks });
    }
    let candidate = Settings { lists, active_list: 0, ..settings.clone() };
...
mod tests {
    ...
    fn imports_paired_and_drop_only_tasks_without_changing_settings() {
        ...
        assert!(parse_task_file(&file.to_string(), &settings).unwrap()[0].tasks[0].course.is_none());
    ...
    fn rejects_duplicate_courses_and_accepts_multiple_watch_lists() {
        ...
        assert_eq!(parse_task_file(&file.to_string(), &Settings::default()).unwrap().len(), 2);
    }
}
