...
确认 package.json、package-lock.json 与 server/Cargo.toml 的版本都是 0.2.0，且对应修改已提交。若已有 v0.2.0 标签，不要删除或强推它；使用递增版本并同步更新锁文件。
...
- 每份包都包含完整 README、LICENSE、CONTRIBUTING 和 docs，不包含个人 data 或编译缓存。
...
- 标签必须是 v 加源码版本，例如 v0.2.0；版本检查不通过时，先同步 package.json、package-lock.json、Cargo.toml 与 Cargo.lock。
