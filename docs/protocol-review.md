...
- `login_qr_cancel` 清除当前账号的扫码状态并使旧请求代次失效。`login_qr_poll` 完成时检查代次、二维码 ID 及账号是否正在执行任务；切换账号时前端清理旧计时器和过期响应。网络请求本身可能继续完成，但过期结果不会写回当前会话。
...
Rust `/api/tasks/stop` 只取消请求所选账号的 token，可中断等待、查询、准备；已经进入 `client.submit` 的操作不被 token 中途丢弃，等待其结果后再停止。这只是进程仍运行时的正常停止路径。

`/api/shutdown` 在任何账号仍运行任务时拒绝退出，全部停止后才发送服务关闭信号；`serve` 没有等待 `tokio::spawn` 的任务句柄，也没有执行“取消并等待所有提交结束”的流程。所以**不能把停止按钮的保证推广到退出程序、杀进程或系统中断**。
...
本次扩展不改学校端点和选退课表单。账号目录由 `accounts.rs` 管理，每个账号独立保存 `AppState`、Cookie Jar、停止信号、课程缓存、设置、UA、凭据和日志；每账号内提交串行，账号之间可以并行。

本机业务请求使用 `X-HDU-Account` 指定账号，未知 ID 返回错误。没有该请求头的旧客户端使用默认账号。`GET/POST /api/accounts` 提供列表和创建，`POST /api/accounts/{id}` 更名；这些都是本机 API，不是学校接口，也不构成调用者身份认证。

默认账号继续使用原 `data/` 文件，新增账号保存到 `data/accounts/<ID>/`。复制账号清单不复制凭据或会话。前端并行启动前逐账号展示清单，使用各账号配置快照分别请求 `/api/tasks/start`；部分失败不会撤回其他已启动账号。

本地模拟服务已验证同时到达的两个账号请求使用独立 Cookie，并生成独立日志；还覆盖单账号停止、持久化、复制和未知账号拒绝。学校真实多会话限制尚未联调。实现入口和使用方法见 [多账号说明](multi-account.md)。
