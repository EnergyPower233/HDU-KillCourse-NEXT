...
| 课程获取 | 同一任务落实端点 | 9999 条单页改为 500 条分页，省略大量空字段和两个非空字段 |
...
| 完成条件 | 单次响应 | 一页不足 500 条；连续 200 个满页则报错，不保存部分结果 |
| 进度 | 日志 | 回调记录页数与原始累计数，可选 total 字段 |
...
`submit_drop` 虽然写有 `Outcome::Rejected` 分支，但当前 `parse_outcome(Action::Cancel, …)` 不会生成它。因此“学校明确拒绝退课后继续下一项”还不是实际可达的提交结果路径。退课**准备失败**会返回 false、跳过配对选课并继续下一项；退课**提交非成功**则停止整轮。

Go `StartWaitCourse` 只检查 `HandleCourse` 是否返回 error；由于明确 `flag=0` 仍返回 nil，可能继续打印“蹲选课成功”、发成功邮件并把该课程标为 0。这是旧版结果传播问题，不能把两版结果处理写成一致。
...
| 恢复 | 配置中存在课程标志更新 | pending/日志在内存；没有持久化执行进度与自动恢复 |
...
| 路径 | 方法 | 用途 |
|---|---|---|
| `/api/health`、`/api/snapshot` | GET | 服务状态与内存运行记录 |
...
当前源码静态计数有 27 个 Rust 测试标记和 5 个前端测试用例。已有用例涉及加密向量、响应分类、课程合并、配置校验、登录顺序、随机数和本地端点；**本次未执行，不能将历史“全部通过”写成本次结论**，也不能替代上述完整流程测试。
...
本次仅重写 Protocol Review，没有修改登录、任务调度或学校请求实现。删除了旧文档中无法由当前源码支持的“完全一致”“无功能影响”“学校协议未变”等断言，也不推断过去某项更改是否经过用户授权。
