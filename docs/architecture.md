...
完整目录地图与调用流程见 [代码结构文档](code-structure.md)。
...
前端入口是 `src/App.tsx`，先读取账号列表，再由 `AccountProvider` 给当前工作台提供固定账号的 API 客户端；工作台负责连接共享状态、页面与弹窗。课程、任务和设置的布局在 `src/pages/`；日志界面在 `src/ActivityLog.tsx`。页面说明集中在 `components/PageHeader.tsx`。

`hooks/useAccounts.ts` 读取全部账号摘要；`hooks/useWorkspace.ts` 读取当前账号数据并轮询状态；`useLogin.ts` 管理账号和扫码流程；`useTaskLists.ts` 管理清单编辑、导入和导出；`useTheme.ts` 同步外观。`domain/` 放不依赖 React 或网络的业务辅助函数，`types.ts` 只放数据类型。

`server/src/accounts.rs` 保存账号目录，每个账号持有独立 `AppState` 和 `Storage`。API 中间件从请求头选择固定账号，未知 ID 不回退到默认账号。后端入口 `server/src/lib.rs` 只负责启动与关闭服务，以及启动时的自动登录。`api/mod.rs` 列出全部本机路由，请求处理按课程、登录、配置、任务和日志划分文件。`client/` 与学校通信，`scheduler.rs` 决定任务如何执行，`storage.rs` 处理本地配置与课程缓存，`run_log.rs` 处理运行日志。
...
- 任务执行必须经过用户确认。每个账号内部余量查询可以并发、提交串行，账号之间独立并行；已发出的选退课请求无法撤回。
- `Success`、`Rejected`、`Unknown` 区分学校响应。`Unknown` 在调度层按本次失败记录为 `failed`，跳过当前课程并继续其他任务，不自动重试；退课未明确成功时跳过该项剩余退课和配对选课。日志保存课程身份和操作类型，不靠当前课程缓存解释历史。
...
多账号的用户流程、请求路由和持久化约定见 [多账号使用说明](multi-account.md)。新增 API 必须明确属于全局管理还是账号数据，不要重新引入全局“当前账号”变量。
