...
// disabled by the app after a failed health check.
...
export const api = {
  ...
  fetchCourses: (settings: Settings) => request<Course[]>('/api/courses/fetch', { body: { settings } }),
  login: (auth: Credentials, settings: Settings) => request<void>('/api/login', { body: { auth, settings } }),
  loginQrStart: (settings: Settings) => request<{ image: string }>('/api/login/qr/start', { body: { settings } }),
  ...
  saveCredentials: (credentials: StoredCredentials) => request<void>('/api/credentials', { body: { credentials } }),
