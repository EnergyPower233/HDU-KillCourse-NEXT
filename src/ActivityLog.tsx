...
export function ActivityLog({ snapshot }: { snapshot: Snapshot }) {
  ...
  useEffect(() => {
    if (snapshot.current_run?.id) {
      setSelected(snapshot.current_run.id);
      setExpanded(false);
    }
  ...
  async function loadOlder() {
    ...
    try {
      ...
      if (selection.current === id) setError(String(e));
    } finally {
  ...
  const choices = [...runs];
  if (snapshot.current_run && !choices.some((r) => r.id === snapshot.current_run?.id))
    choices.unshift(snapshot.current_run);
  return (
    <>
      <div className="panel-heading">
        <div>
          <h2>运行记录</h2>
          <p>每次运行单独保存到本地 · 新任务不会删除历史记录</p>
        </div>
        <span className={`badge ${live ? 'running' : 'neutral'}`}>
          {live ? '本次正在运行' : '历史记录'}
        </span>
      </div>
          ...
          >
            ...
            {choices.map((r) => (
              <option key={r.id} value={r.id}>
                {new Date(r.started_at).toLocaleString()} · {r.list_name} · {r.id.slice(-6)}
              </option>
            ))}
          </select>
        </label>
      ...
      {error && (
        <p role="alert" className="run-log-error">
          {error}
        </p>
      )}
      {warning && <p role="status">{warning}</p>}
      {expanded && live && (
        <p className="run-log-hint">正在查看较早记录，实时刷新已暂停；点击「查看最新记录」恢复。</p>
      )}
      {loading ? (
        <p className="run-log-hint" role="status">
          正在读取日志…
        </p>
      ) : events.length ? (
        <div className="log-list">
          {events
            ...
            .reverse()
            .map((e, i) => (
              <div className="log-row" key={`${selected}-${events.length - i}`}>
                <time dateTime={e.time}>
                  {e.time.includes('T') ? new Date(e.time).toLocaleString() : e.time}
                </time>
                <span className={`badge ${e.status}`}>{progressLabel(e)}</span>
                <div>
                  {e.course_id && (
                    <strong className="log-course-name">{e.course_name || e.course_id}</strong>
                  )}
                  {e.message}
                  {e.course_id && (
                    <small>
        ...
        <p className="run-log-hint">
          {selected
            ? '这次运行暂时没有事件。'
            : '开始任务后，每次运行都会保存在这里，重启程序后仍可查看。'}
        </p>
      )}
      {before !== null && (
        <div className="run-log-controls">
